...
describe('classTutOverviewRoutes', () => {
  ...
  it('should call getClassTutOverview controller with the correct teacherId', async () => {
    const getClassTutOverviewStub = sinon.stub(classTutOverviewController, 'getClassTutOverview').callsFake((req, res) => {
    ...
    await request(app)
      ...
      .then((res) => {
        ...
        expect(getClassTutOverviewStub.firstCall.args[0].params.teacherId).to.equal(teacherId);
