...
describe("Course Controller", () => {
  ...
  describe("createNewCourse", () => {
    it("should create a new course and return status 201", async () => {
      ...
      expect(res.json.calledWith({ message: "Course created!", course: req.body })).to.be.true;
    });

    it("should return status 500 on error", async () => {
      sinon.stub(courseServices, "createNewCourse").throws(new Error("Error creating course"));

      await createNewCourse(req, res);

      expect(res.status.calledWith(500)).to.be.true;
      expect(res.json.calledWith({ message: "Error creating course", error: "Error creating course" })).to.be.true;
    });
  });
  ...
  describe("getAllCourses", () => {
    ...
    it("should return status 500 on error", async () => {
      sinon.stub(courseServices, "getAllCourses").throws(new Error("Error fetching courses"));
  ...
  describe("getCourseById", () => {
    ...
    it("should return status 500 on error", async () => {
      sinon.stub(courseServices, "getCourseById").throws(new Error("Error fetching course"));

      await getCourseById(req, res);

      expect(res.status.calledWith(500)).to.be.true;
      expect(res.json.calledWith({ message: "Error fetching course", error: "Error fetching course" })).to.be.true;
    });
  });

  describe("updateCourse", () => {
    it("should update a course by ID and return status 200", async () => {
      const updatedCourse = { ...req.body, courseName: "Updated Software Engineering" };
      ...
      expect(res.status.calledWith(200)).to.be.true;
      expect(res.json.calledWith({ message: "Course updated successfully", course: updatedCourse })).to.be.true;
    });
    ...
    it("should return status 500 on error", async () => {
      sinon.stub(courseServices, "updateCourse").throws(new Error("Error updating course"));

      await updateCourse(req, res);

      expect(res.status.calledWith(500)).to.be.true;
      expect(res.json.calledWith({ message: "Error updating course", error: "Error updating course" })).to.be.true;
    });
  });
  ...
  describe("deleteCourse", () => {
    ...
    it("should return status 500 on error", async () => {
      sinon.stub(courseServices, "deleteCourse").throws(new Error("Error deleting course"));

      await deleteCourse(req, res);

      expect(res.status.calledWith(500)).to.be.true;
      expect(res.json.calledWith({ message: "Error deleting course", error: "Error deleting course" })).to.be.true;
    });
  });
  ...
  describe("getCoursesBySemester", () => {
    ...
    it("should return status 500 on error", async () => {
      sinon.stub(courseServices, "getCoursesBySemester").throws(new Error("Error fetching courses"));
