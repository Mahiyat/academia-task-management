 ...
 * @property {Array<object>} coursesProgress -
 * An array containing the progress details of each course.
 ...
 * @property {string} coursesProgress[].lectureProgress -
 * The lecture progress of the course as a percentage (0-100).
 * @property {string} coursesProgress[].tutorialProgress -
 * The tutorial progress of the course as a percentage (0-100).
 */
export const getSemesterProgress = async (semesterId) => {
  ...
  const coursesProgress = await Promise.all(
    semester.courses.map(async (course) => {
      ...
      const lectureProgress =
        (courseData.noOfClassesTaken / courseData.expectedNoOfClasses) * 100 ||
        0;
      const tutorialProgress =
        (courseData.noOfTutorialsTaken / courseData.expectedNoOfTutorials) *
          100 || 0;
  ...
  const overallProgress =
    ((totalClassesTaken + totalTutorialsTaken) /
      (totalClassesExpected + totalTutorialsExpected)) *
      100 || 0;
